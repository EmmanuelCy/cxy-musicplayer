...
def get_lyrics(artist, title):
    ...
    request_url = f"{base_url}{artist}/{title}"
...
def fetch_lyrics():
	song = dpg.get_value("##song")
	artist, title = map(str.strip, song.split('-'))
	lyrics = get_lyrics(artist, title)
	dpg.set_value("##OutputText", lyrics)
...
with dpg.window(tag="main",label="window title"):
	...
	with dpg.group(horizontal=True):
		with dpg.child_window(width=400,tag="sidebar"):
			...
			dpg.add_separator()
			dpg.add_spacer(height=5)
			dpg.add_input_text(hint="Search for Lyrics: ex.Maroon 5 - Sugar",width=-1, id="##song", callback=fetch_lyrics)
			# dpg.add_button(label="Fetch Lyrics", callback=fetch_lyrics)
			dpg.add_text("Lyrics:", tag="lyrics")
			dpg.add_text(label="", id="##OutputText", wrap=370)
